...
type ClaimedHistoryData = {
  amount: bigint
  timestamp: bigint
}

export const CompensationClaimTable: FC<{
  contractData?: any
}> = ({ contractData = [] }) => {
  ...
  const data = useMemo(() => {
    if (contractData) {
      return (contractData as ClaimedHistoryData[]).map((item, index) => {
        const dateNum = Number(item.timestamp ?? 0)
        return {
          id: index,
          claimAmount: formatCurrency(Number(item.amount?.toString() ?? 0)),
          date:
            dateNum > 0
  ...
  return (
    ...
    >
      <TableHeader>
        ...
        <TableColumn key='claimAmount'>{t('columns.claimAmount')}</TableColumn>
      </TableHeader>
