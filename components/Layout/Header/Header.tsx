...
const Header: FC<HeaderProps> = ({ className }) => {
  ...
  return (
    <div className={clsx(
      'flex items-center w-full border-co-border-1 bg-opacity-5 backdrop-blur-md py-3 md:py-6 px-4 md:px-[60px] justify-between rounded-[32px] border',
      className
    )}>
      <div className=''>
        <Link href='/'>
      ...
      <div className='hidden md:flex items-center justify-center gap-[80px]'>
        {MenuList.map(({ key, label, href }) => {
          return <Link className={
            clsx(
              'text-co-text-1 text-xl pb-3 pt-1',
              asPath === href && 'border-b border-co-border-1 font-semibold'
            )
          } href={href} key={key} >{label}</Link>
        })}
      </div>
            ...
            <div className='border-gradient w-full h-full flex flex-col gap-7 items-center justify-start py-[30px] px-[25px]'>
...

              {
                isAuthenticated && isConnected ?
                  <>
                    <Button onClick={handleMyAccountButtonClick} color='primary' className='w-full h-[50px] !rounded-full text-[20px]'>My Account</Button>
                    <div className='w-full'>
                      <Divider className='bg-co-bg-3' />
                      <div onClick={handleLogout} className='cursor-pointer mt-5 text-center w-fit px-12 mx-auto text-xl'>Log out</div>
                    </div>
                  </> :
                  <Button onClick={handleConnectWalletButtonClick} className='w-full h-[50px] !rounded-full text-[20px]'>Connect Wallet</Button>
              }
            </div>
          </PopoverContent>
        </Popover>
